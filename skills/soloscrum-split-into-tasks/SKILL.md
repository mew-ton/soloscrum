...
2. **Verify `/soloscrum:breakdown` is the right command** (per `soloscrum-define-issue-size`):
   - If the Issue likely bundles multiple intents (SP > 5 or `/soloscrum:breakdown` would produce > 5 Subtasks — both read as mis-scope smells), route back to `/soloscrum:refine` for Issue split rather than proceed.
   - If the Issue's intent is coherent and a single PR would be reviewable, skip `/soloscrum:breakdown` and go directly to `/soloscrum:develop` (per `CLAUDE.md`'s *"Issues that fit within a single develop unit can skip this step"*).
...
5. If subtask count exceeds `max_subtasks` (5) in `soloscrum-define-issue-size`, treat as a mis-scope smell — the Issue likely bundles multiple intents. Return to `/soloscrum:refine` rather than proceed.
...
- `soloscrum-define-issue-size` (when `/soloscrum:breakdown` is the right command, and the mis-scope-smell threshold)
