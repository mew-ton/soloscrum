...
|---|---|---|
| Product Owner | `soloscrum-po` | `/soloscrum:refine` |
| Design | `soloscrum-design` | `/soloscrum:validate`, `/soloscrum:breakdown` (plan stage) |
| Dev | `soloscrum-dev` | `/soloscrum:breakdown` (register stage), `/soloscrum:develop` |
| UI | `soloscrum-ui` | `/soloscrum:design-ui` |
| Review | `soloscrum-review` | `/soloscrum:review` |
...
|---|---|---|---|
| Issue (parent) | po | po (refine; close via `/soloscrum:refine` janitor when sub-issues are all done) — agents do **not** close at verdict time; merge-time GH auto-close + janitor cover this (see `soloscrum-define-pr-lifecycle`, "Issue close happens at merge") | review |
...
| Subtask record | dev (during `/soloscrum:breakdown`) | dev/ui (own state), review (Done) | review |
