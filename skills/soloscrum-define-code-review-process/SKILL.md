...
description: "Reference: standard code review pipeline (CodeRabbit + multi-agent review + the user's review perspectives) and the per-finding decision rules. Severity and confidence scores are inputs to a per-item decision, NOT auto-skip filters. Every CodeRabbit finding is surfaced and judged individually; agent-generated findings carry a confidence pre-filter, and perspective findings carry a grounding requirement — both suppress hallucinations without silently dropping signal."
...
When the corpus is **not** empty, report how many perspectives it holds and how many were selected, even when none were. A perspective whose trigger is too narrow to ever fire again is otherwise indistinguishable from one that is simply not relevant today — the corpus can rot silently, and this line is the only signal that would surface it.
...
### Review-perspective findings — grounding requirement instead of a confidence score

The perspective itself is trusted: the user authored it and confirmed it when it was collected. **A finding it produces is not** — that finding was generated by an LLM reading a diff, the same mechanism the 80-confidence filter exists to guard. "The rule is sound" and "this instance of the rule is real" are different claims, and conflating them would give perspective findings more trust than agent findings while using the identical generation path.

So perspective findings take no confidence *score*, but they must be **grounded**:

- Every perspective finding cites **the diff location** (file and line, inside this PR's diff) and **the specific check in the perspective body** it instantiates.
- A finding that cannot cite both is **discarded as unsupported** — not skipped-with-reason, because there is nothing to decide on. Discarding here targets hallucinated instances specifically, rather than filtering by a score that would also drop correct low-confidence ones.
- Grounded findings then go through the same per-item decision as every other source. No severity or confidence threshold applies.

This is deliberately not the 80-confidence filter. That filter drops findings a scoring agent is unsure about, which for a perspective would mean discarding exactly the judgement the user collected it to preserve. The grounding requirement instead demands evidence, which a hallucinated instance cannot supply and a real one always can.
...
- Review perspectives take neither filter. Applying the 80-confidence filter would discard exactly the judgements the user collected them to preserve; applying nothing would trust an LLM-generated instance because the *rule* behind it was authored deliberately. The grounding requirement above is the third answer: demand evidence rather than a score.
...
<corpus line: N perspectives available, M selected for this PR — omit entirely when N is 0>
...
**Consolidate across sources before posting.** Perspectives deliberately encode the same class of judgement some multi-agent lenses already cover, so the same defect can arrive from two or three sources. Report it **once**, in the section of the source that described it best, listing the other sources that also raised it. Deciding the same finding independently in three places produces redundant noise at best and contradictory actions at worst.
