...
description: "Reference: story point scale and estimation criteria. SP is anchored on scope x uncertainty (size class, not a time unit); SP 5 is the upper bound and Issues estimated above it are mis-scope smells signalling likely intent bundling — return to /soloscrum:refine for Issue split per soloscrum-define-issue-size."
...
- **Owner**: `soloscrum-po` (during `/soloscrum:refine`)
...
When Issue SP exceeds the threshold, treat as a mis-scope smell — the Issue likely bundles multiple intents — and return to `/soloscrum:refine` for Issue split per `soloscrum-define-issue-size`, then re-estimate each split Issue. (A coherent single intent that genuinely needs > 5 SP worth of work is delivered through `/soloscrum:breakdown` Subtask slices, not by Issue split — see `soloscrum-define-issue-size` for the distinction.)

### Subtask SP (Dev Layer)

- **Purpose**: The actual value registered in the tracker, used for planning and progress tracking
- **Owner**: `soloscrum-dev` (during `/soloscrum:breakdown`)
...
| >5 | (mis-scope signal) | (mis-scope signal) | Do not register — treat as "this Issue likely bundles multiple intents," return to `/soloscrum:refine` and split per `soloscrum-define-issue-size`, re-estimate each split Issue. A coherent single intent that genuinely needs >5 SP worth of work is delivered through `/soloscrum:breakdown` Subtask slices rather than by Issue split. |
...
4. If SP > 5, treat as a mis-scope smell — return to `/soloscrum:refine` and split per `soloscrum-define-issue-size`. A coherent single intent that genuinely needs > 5 SP worth of work goes to `/soloscrum:breakdown` (delivery slicing) instead, not Issue split.
