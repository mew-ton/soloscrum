...
| >5 | (予算超過) | (予算超過) | 割り当てない — [`issue-size`](/ja/policies/issue-size/) に従って split し再見積もり |

## 2 つの段階

SP はライフサイクルの 2 箇所に登場します:

**Issue SP** は粗さチェックです。PO は `/refine` 中に、Issue がライフサイクルに入れるほど小さいか、それとも先に [`issue-size`](/ja/policies/issue-size/) が `suggest_split` を発火する必要があるかを判断するためにこれを使います。これはどこにも保存され **ません** — レコードではなく、決定への入力です。
...
4. 結果が 5 を超える場合、Issue は予算超過 — [`issue-size`](/ja/policies/issue-size/) に従って split し再見積もりする。

## 関連項目

- tracker profile ごとの SP 値の保存場所: [tracker profile](/ja/concept/tracker-profile/)。
