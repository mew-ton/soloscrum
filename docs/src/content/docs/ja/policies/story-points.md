...
**Issue SP** はざっくり度合いのチェックだ。PO は `/refine` で、Issue がそのままライフサイクルに乗せられるサイズか、それとも先に [`issue-size`](/ja/policies/issue-size/) が `suggest_split` を発火させる必要があるかを判断するためにこれを使う。値はどこにも記録され **ない** — あくまで決定のための入力であって、tracker に残るレコードではない。
...
3. (scope, uncertainty) を SP 表にマッピングする。両軸とも収まる必要がある。迷ったら大きい側の SP を選ぶ。
