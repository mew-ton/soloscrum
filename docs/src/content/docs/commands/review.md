...
- For design-fidelity review of a Figma file (`type:design-ui` Subtasks), pass the Figma URL.
...
7. **Post-verdict actions.** On Pass: approve the PR (`gh pr review --approve`, see the self-approve note below), transition the Subtask to `done`, wait for CI green, promote the PR to ready (`gh pr ready`), and surface the merge command. On Fail: post per-finding feedback, revert the Subtask to `in-progress`, leave the PR in draft.
