...
Story points (SP) measure scope × uncertainty — not time. soloscrum uses a 1/2/3/5 scale; SP > 5 means the Issue must be split before `/develop` runs.
...
SP shows up at two points in the lifecycle:
