...
| >5 | (mis-scope signal) | (mis-scope signal) | Treat as "this Issue likely bundles multiple intents" — return to `/soloscrum:refine` and split per [`issue-size`](/policies/issue-size/). Re-estimate per split Issue. |

## The two levels

SP appears at two points in the lifecycle:

**Issue SP** is a size check. The PO uses it during `/soloscrum:refine` to decide whether the Issue is small enough to enter the lifecycle, or whether [`issue-size`](/policies/issue-size/) needs to fire `suggest_split` first. Issue SP is **not** stored — it is a decision input, not a record.

**Subtask SP** is the actual recorded value. Dev calculates it during `/soloscrum:breakdown` from the Subtask's slice scope — its "what" and any checklist items per [`issue-format`](/policies/issue-format/)'s Subtask body; Subtasks do not carry their own AC — and writes it to the tracker via `soloscrum-tracker-{github|linear}-set-sp`. This value is used for backlog planning and progress tracking.
...
4. If the result exceeds 5, treat as a mis-scope smell — the Issue likely bundles multiple intents. Return to `/soloscrum:refine` and split per [`issue-size`](/policies/issue-size/), then re-estimate each split Issue.
