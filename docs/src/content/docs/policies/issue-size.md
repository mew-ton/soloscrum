---
title: Issue size
description: Issue and Subtask split criteria operate on intent coherence, not raw work volume. SP > 5 and subtask count > 5 are mis-scope smells. /soloscrum:breakdown fires when one PR would be unreviewable.
sidebar:
  order: 4
---
...
A large but coherent single intent is not split. Its delivery is sliced by `/soloscrum:breakdown` into multiple reviewable PRs (Subtasks).
...
| Subtask count | > 5 | If one intent needs more than ~5 reviewable PRs to deliver, it is probably multiple intents combined by mistake. Mis-scope smell — return to `/soloscrum:refine`. |
| Estimated days | > 2 | Coarse calibration only. Does not by itself force a split. |

If the answer to *"are these multiple intents?"* is no — this is one coherent intent that just happens to be large — the Issue stays. The work is then handled by `/soloscrum:breakdown` into Subtasks (delivery slices), not by Issue split.
...
`/soloscrum:breakdown` produces Subtasks when delivering one Issue's intent as a single PR would produce an unreviewable PR — a delivery / reviewability question.

- One PR is reviewable → no `/soloscrum:breakdown` needed; go directly to `/soloscrum:develop`. (`CLAUDE.md` states the same: *"Issues that fit within a single develop unit can skip this step"*; branch-per-Issue mode in the canonical [`soloscrum-define-branch-commit`](https://github.com/mew-ton/soloscrum/blob/main/skills/soloscrum-define-branch-commit/SKILL.md).)
- One PR would not be reviewable → `/soloscrum:breakdown` produces Subtasks, each carrying a reviewable slice. Subtasks slice the *delivery*, not the intent — see [`issue-format`](/policies/issue-format/)'s Subtask body section.
...
- **Phase axis** — only when the phase has its own independent "done" (e.g. performance hardening with its own SLO that can be verified on its own terms). A phase with no done of its own is delivery work, not a separate intent — it belongs in `/soloscrum:breakdown`, not Issue split.
...
`max_estimated_days` is a coarse calibration check during `/soloscrum:refine`. If the rough wall-clock feel obviously exceeds two days of solo-dev cycle time (including user review), that is a signal the scope / uncertainty estimate is probably too low. Not the primary criterion.
