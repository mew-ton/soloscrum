...
6. Always review the Subtask's slice scope (its "what" + Checklist) and the **parent Issue's AC** before starting implementation — Subtasks do not carry their own AC per `soloscrum-define-issue-format`'s Subtask Body section; the parent owns the AC and the Subtask delivers a slice toward it
